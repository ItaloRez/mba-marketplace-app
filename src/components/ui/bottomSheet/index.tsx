...
export const BottomSheetPortal = ({
  ...
  const { bottomSheetRef, handleClose, visible } =
  ...
  return (
    <Portal isOpen={visible} animationPreset="slide">
...
export const BottomSheetTrigger = ({
  ...
  // If asChild is true, clone the first child and pass it the necessary props
  ...
  // Default Pressable implementation
  return (
    <Pressable
      onPress={(e) => {
...
const bottomSheetCloseStyle = tva({
  base: 'p-2 rounded-full hover:bg-background-50 active:bg-background-100',
})
...
export const BottomSheetClose = ({
  ...
  const { handleClose, bottomSheetRef } = useContext(BottomSheetContext)

  return (
    <Pressable
      {...props}
      className={bottomSheetCloseStyle({
        className: className,
      })}
      onPress={(e) => {
        console.log('close onpress')
        handleClose()
        props.onPress && props.onPress(e)
      }}
      role="button"
